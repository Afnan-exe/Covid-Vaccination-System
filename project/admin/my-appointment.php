                ...
                <table class="table mb-0">
                    <tbody>
                        ...
                        <tr>
                            <?PHP
                             $ID = $_SESSION['ID'] ;
                            $select = "SELECT A.ID , H.HOSPITAL_NAME , V.VACCINE , A.DATE , U.NAME , A.STATUS FROM `appointment`AS A JOIN users AS U ON U.ID = A.USER_ID JOIN `vaccination` AS V ON V.ID = A.VACCINE_ID JOIN `hospital` AS H ON H.ID = A.HOSPITAL_ID WHERE `USER_ID` = $ID";
